    ...
    with po:
        ...
        if base is None or base.empty:
            ...
            if pool.empty:
                ...
                sectors = pool["Sector"].astype(str).tolist()
                sector_count = max(len(set(sectors)), 1)
                requested_cap = sector_cap_pct / 100.0
                min_feasible_cap = 1.0 / float(sector_count)
                effective_cap = requested_cap
                cap_relaxed = False
                if requested_cap < min_feasible_cap:
                    effective_cap = min_feasible_cap
                    cap_relaxed = True
                    st.warning(
                        f"Requested sector cap ({sector_cap_pct:.0f}%) is infeasible for this universe "
                        f"({sector_count} sector{'s' if sector_count != 1 else ''}). "
                        f"Using effective cap {effective_cap*100:.1f}%."
                    )

                w = _bounded_weights(raw, min_w_pct / 100.0, max_w_pct / 100.0)
                for _ in range(6):
                    prev = w.copy()
                    w = _enforce_sector_cap(w, sectors, effective_cap)
                    w = _bounded_weights(w, min_w_pct / 100.0, max_w_pct / 100.0)
                    if np.max(np.abs(w - prev)) < 1e-8:
                        break
                ...
                sec_totals = out.groupby("Sector", dropna=False)["Optimized (%)"].sum().sort_values(ascending=False)
                if not sec_totals.empty:
                    realized_max = float(sec_totals.iloc[0])
                    cap_label = effective_cap * 100.0
                    msg = f"Largest sector weight: {realized_max:.1f}% (cap target: {cap_label:.1f}%)."
                    if cap_relaxed:
                        st.caption(msg + " Cap was auto-relaxed to satisfy feasibility.")
                    else:
                        st.caption(msg)
